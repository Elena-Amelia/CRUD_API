...
export async function controller(req: IncomingMessage, res: ServerResponse) {
  ...
  const [api, user, id, ...rest] = req.url.split("/").slice(1);
  // const arr = req.url.split("/").slice(1);;
  //   console.log(api, user, id, ...rest);
  // let id
  //   if (`${api}/${user}` === 'api/user' && !(rest))
  ...
  try {
    switch (req.method) {
      ...
      case HTTP_METHODS.POST:
        if (id) {
          result = {
            error: "You are not allowed to send id to endpoint in method POST",
          };
